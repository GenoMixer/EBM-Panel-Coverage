...
# target directory
ebm_target_dir="/mnt/ngs-resources/transcript_annotations/refseq_hg19/EBM-Panel/"
...
rule mosdepth:
    ...
    params:
        extra="--fast-mode --mapq 20 --thresholds 0,1,30,1000",
        prefix="{sample}"
    threads: 4
    shell: 
        """
...
rule create_bed:
    ...
    output: "coverage/{sample}.targets_pct_gt_30.bed"
    shell:
        """
...
rule create_bed_two:
    input: "coverage/{sample}.targets_pct_gt_30.bed"
    output: "coverage/{sample}.targets_not_100pct_gt_30.bed"
    shell:
        """
...
rule create_bed_r:
    input: "coverage/{sample}.targets_not_100pct_gt_30.bed"
    output: "coverage/{sample}.targets_not_100pct_gt_30.txt"
    shell: 
        """
...
rule intersect:
    input:
        ...
        bbed="coverage/{sample}.targets_not_100pct_gt_30.bed"
    output: "coverage/{sample}.bases_lt_30_1based.txt"
    shell: 
        """
        bedtools intersect \
           -a {input.abed} \
           -b {input.bbed} \
        | awk -v desired_min_cov="30" 'BEGIN {{FS="\t"; OFS="\t"}} {{if ($4 < 500) print $0}}' \
...
rule intersect_two:
    input: 
        abed="coverage/{sample}.bases_lt_30_1based.txt",
        bbed=os.path.join(ebm_target_dir, "EBM-Panel.hg19.selected_transcripts_cds.sorted.int20.bed.all_bases_annotated.txt")
    output: "coverage/{sample}.bases_lt_30_1based_annotated.txt"
    shell:
        """
        bedtools intersect \
           -a {input.abed} \
...
rule sort:
    input: "coverage/{sample}.bases_lt_30_1based_annotated.txt"
    output: "coverage/{sample}.bases_lt_30_1based_annotated_regions.txt"
    shell:
        """
...
rule basecov:
    input:
        genes="coverage/{sample}.genelist",
        regions="coverage/{sample}.bases_lt_30_1based_annotated.txt"
    output: "coverage/{sample}.bases_lt_30_1based_annotated_genelist.txt"
    shell:
        """
        set -e \
        grep -F \
...
rule regions:
    input:
        genes="coverage/{sample}.genelist",
        regions="coverage/{sample}.bases_lt_30_1based_annotated_regions.txt"
    output: "coverage/{sample}.bases_lt_30_1based_annotated_regions_genelist.txt"
    shell:
        """
        set -e \
        grep -F \
...
rule base:
    input:
        genes="coverage/{sample}.genelist",
        target="coverage/{sample}.targets_not_100pct_gt_30.txt"
    output: "coverage/{sample}.targets_not_100pct_gt_30_genelist.txt"
    shell: 
        """
        set -e \
        grep -F \
        ...
        {input.target} \
        | awk \
        -v desired_min_cov="30" \
        'BEGIN {{FS="\t"; OFS="\t"; print "Chr","Start","End","Target_Name","pct_bases_gt_30x_cov"}} {{print $0}}' - \
        > {output}
        """
...
rule rscript:
    input: 
        ...
        pct="coverage/{sample}.targets_not_100pct_gt_30_genelist.txt",
        regions="coverage/{sample}.bases_lt_30_1based_annotated_regions_genelist.txt",
        bases="coverage/{sample}.bases_lt_30_1based_annotated_genelist.txt"
    output: 
